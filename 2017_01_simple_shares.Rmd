...
* FY2016 HUD funding allocation
https://www.hud.gov/program_offices/comm_planning/budget/fy16
* FY2015 CDBG calculation workbook (emailed from HUD)
...
```{r}
pprn_df <- read.csv("data/inputs/hud_allocations/FY-2016-Geographic-Codes-with-PPRN.csv") %>%
  clean_names() %>% 
...
#import ESG17 allocations from HUD to check for any new ESGs that aren't in 2016's PPRN list as these will get dropped mistakenly.
hud_funding <- read_excel("data/inputs/hud_allocations/fy2017-formula-allocations-AllGrantees.xlsx", skip = 1) %>%
  clean_names() %>%
  dplyr::select (key, name, sta, esg17_total) %>%
  rename(UOGID = key,
         esg17 = esg17_total) %>%
...
```{r}
#Importing CDBG crosswalk from Todd Richardson and adjusting to 2017
crosswalk <- read_excel("data/inputs/hud_calculations/Disaggregated COC to ESG crosswalk.xlsx") %>%
  clean_names() %>% 
  rename (UOGID = geocode,
          coc_num = coc_id) %>% 
...
#ESG removals for 2017
    ...
    UOGID == "261638" ~ "269999", #Add Dearborn to MI BoS
...
#import ESG16 allocations from HUD to make list of eligible UOGIDs
hud_funding <- read_excel("data/inputs/hud_allocations/fy2016-formula-allocations-AllGrantees.xlsx", skip = 1) %>% 
  ...
  rename(UOGID = key) %>%
  filter (esg16 != 0) %>% #finds only ESG eligible
...
#import ESG16 allocations from HUD to calculate total allocation
hud_funding <- read_excel("data/inputs/hud_allocations/fy2016-formula-allocations-AllGrantees.xlsx", skip = 1) %>%
  dplyr::select (1, 8) %>%
  clean_names() %>%
  rename(UOGID = key)
...
esg_total <- sum (hud_funding$esg16) * .998
```
...
```{r}
cdbg_allocation <- read_excel ("data/inputs/hud_calculations/CDBG FY15.xlsx", sheet = "CALCS") %>% 
  rename (UOGID = Key,
          cdbg_amt = "GA15") %>% 
  dplyr::select (UOGID, NAME, cdbg_amt)

#calculate total 2016 CDBG formula allocations
...
write.csv (esg_coc_crosswalk, "data/outputs/x17_simple_shares.csv")
```