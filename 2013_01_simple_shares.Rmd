...
## Sources 
* FY 2012 Continuum of Care (CoC) Geographic Code Report
https://www.yumpu.com/it/document/view/22352342/preliminary-pro-rata-need-onecpd
* FY2012 HUD funding allocation
https://www.hud.gov/program_offices/comm_planning/budget/fy12
* FY2011 CDBG calculation workbook (emailed from HUD)
...
```{r}
pprn_df <- read.csv("data/inputs/hud_allocations/FY-2012-Geographic-Codes-with-PPRN.csv") %>%
  clean_names() %>% 
  rename (UOGID = geo_code,
          pprn_2 = pprn_fy2012,
          name = jurisdiction_name) 
#%>% 
#  rbind (c("139003", "GA", "Atkinson County", "$35,426")) #adds row Tabula missed
...
#ESG removals for 2017
    UOGID == "061750" ~ "069999", #Add Irvine to CA BoS
    UOGID == "269081" ~ "269999", #Add Kent County to MI BoS again
    UOGID == "366468" ~ "369999", #Add Tonawanda Town to NY BoS
...
#ESG removals for 2013
    ...
    UOGID == "363160" ~ "369999", #Add Islip Town to NY BoS
    UOGID == "423573" ~ "429999", #Add Lancaster to PA BoS
    UOGID == "511320" ~ "379999", #Add Roanoke to NC BoS
...
#ESG additions going from 2020 to 2019
    ...
    #UOGID == "069111" ~ "069111", #Ventura County returns to CA BoS for 2013/2014
...
#ESG additions for 2016
    ...
    UOGID == "069083" ~ "069083", #Separate Santa Barbara County again   
    UOGID == "089001" ~ "089001", #Separate Adams County from CO BoS
  #  UOGID == "342454" ~ "342454", #Passaic City returns to NJ BoS in 2013
...
#ESG additions for 2013
    ...
    UOGID == "183042" ~ "183042", #Separate Terre Haute from IN BoS
    UOGID == "729013" ~ "729013", #Separate Arecibo Municipio from PR BoS
    UOGID == "491014" ~ "491014", #Separate Provo from UT BoS
    UOGID == "483330" ~ "483330", #Separate McAllen from TX BoS
      TRUE ~ esg_id
  )) %>%
  
  #FIX MAJOR COC CHANGES HERE or else they won't get homeless counts in 02_formula program
  #https://files.hudexchange.info/resources/documents/FY2012_CoCNamesNumbers.pdf (See this for proper placement)
  mutate (coc_num = case_when ( 
    ...
    UOGID %in% (c("369035", "369057", "369095")) ~ "NY-506", #Fulton, Montgomery, Schoharie Counties CoC
    UOGID == "069015" ~"CA-528", #Del Norte County COC
    TRUE ~ coc_num)) %>%
  
  #mutate (UOGID = if_else(str_detect(UOGID, "110006"), "119999", UOGID)) %>%
  #Add other cities not present in 2020 crosswalk, but added to CDBG geographies in previous years
...
#Check for in PPRN and missing from crosswalk -- this will be those areas without CoCs (several places in AL, FL, & OK), insular areas and a few cities that have with joint agreements with their county. Should be 0 after adds. #Middlesex County MA sticks
anti_join (pprn_df, crosswalk, by = "UOGID") %>%
  filter(!(UOGID %in% c("129003", "129029", "129125", "010072", "010882", "019015", "019019","019049","019055","050930","059033","059047","059083","059113","059127","059131", "409087","409149"))) %>% #filter out non-COC counties in AL, AR, FL, OK
  filter(!(state %in% c("AS","GU","MP","VI"))) #filter out insular areas 
```
...
#import ESG12 allocations from HUD to make list of eligible UOGIDs
hud_funding <- read_excel("data/inputs/hud_allocations/FY2012_20383_ALLGRANTEES.XLS", skip = 1) %>%
  ...
  dplyr::select (1, 5, 7) %>%
  filter (esg12 != 0) %>% #finds only ESG eligible
...
#re-import ESG12 allocations from HUD to calculate total allocation
hud_funding <- read_excel("data/inputs/hud_allocations/FY2012_20383_ALLGRANTEES.XLS", skip = 1) %>%
  clean_names() %>%
  rename(UOGID = key) %>%
  mutate (UOGID = if_else (name == "District Of Columbia", "119999", UOGID)) %>% 
  dplyr::select (UOGID, esg12)

#Calculate total ESG-entitled allocation
esg_total <- sum (hud_funding$esg12) * .998
```

## Calculating ESG/CDBG entitled areas from CDBG16
This imports the CDBG12 calculations worksheet from HUD and calculates a total allocation amount. 
...
2011's data for states contains far more than their normal allocation. It looks like they've summed all jurisdictions in the state rather than breaking it out as the state nonentitlement funding. This code strips all states out and adds back in the $989 million allocated to them to total the whole to year's allocation of $3.303 billion.
Source: [Congressional Service Report](https://www.hsdl.org/?view&did=750383) by Eugene Boyd
```{r}
cdbg_allocation <- read_excel ("data/inputs/hud_calculations/CDBG FY11.xlsx") %>%
  filter (TY %in% c("21", "51", "52", "66")) %>% #keep only states, cities and counties
  mutate (HPL = if_else (HPL == "0000", "9999", HPL),
          UOGID = paste0(ST, HPL)) %>%
  rename (cdbg_amt = "GA") %>% 
  dplyr::select (UOGID, TY, NAME, cdbg_amt) %>% 
  filter (!(str_detect(UOGID, "9999$")))

#calculate total 2011 CDBG formula allocations
cdbg_total <- sum (cdbg_allocation$cdbg_amt) + 989000000
...
#Check my calculated ESG funding adds up to approximately the FY2014 ESG allocation of $250M. Will be different because of all the going between geographies. 
...
write.csv (esg_coc_crosswalk, "data/outputs/x13_simple_shares.csv")
```