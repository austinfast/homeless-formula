...
## Sources 
* FY 2011 Continuum of Care (CoC) Geographic Code Report
https://files.hudexchange.info/resources/documents/FY2011_PPRNAmts.pdf
* FY2011 HUD funding allocation
...
```{r}
pprn_df <- read.csv("data/inputs/hud_allocations/FY-2011-Geographic-Codes-with-PPRN.csv") %>%
  clean_names() %>% 
...
#ESG removals for 2017
 ...
 #   UOGID == "366468" ~ "369999", #Separate Tonawanda Town again for 2012
    UOGID == "459063" ~ "459999", #Add Lexington County to SC BoS
#   UOGID == "261638" ~ "269999", #Dearborn separates again
...
#ESG removals for 2013
 ...
 #  UOGID == "363160" ~ "369999", #Separate Islip Town again for 2012
    UOGID == "423573" ~ "429999", #Add Lancaster to PA BoS
 #   UOGID == "511320" ~ "379999", #Roanoke reseparates in 2012
...
#ESG removals for 2012
    ...
    UOGID == "399113" ~ "399999", #Add Montgomery County to OH BoS
...
#ESG additions for 2012
    ...
    UOGID == "480402" ~ "480402", #Separate Beaumont from TX BoS
...
hud_funding <- read_excel("data/inputs/hud_allocations/FY2011_20383_ALLGRANTEES.XLS", skip = 1) %>%
  ...
  mutate (UOGID = case_when (
    ...
    TRUE ~ UOGID)) %>% 
  dplyr::select (1, 5, 9) %>%
  filter (esg11 != 0) %>% #finds only ESG eligible
...
#re-import ESG11 allocations from HUD to calculate total allocation
hud_funding <- read_excel("data/inputs/hud_allocations/FY2011_20383_ALLGRANTEES.XLS", skip = 1) %>%
  clean_names() %>%
  rename(UOGID = key,
         esg11 = esg11_total) %>%
  mutate (UOGID = case_when (
    ...
    name == "MAUI COUNTY" ~ "150003", 
    TRUE ~ UOGID)) %>% 
  dplyr::select (UOGID, esg11) 

#Calculate total ESG-entitled allocation - should be $250M
esg_total <- sum (hud_funding$esg11) * .998
```
...
```{r}
cdbg_allocation <- read.csv ("data/inputs/hud_calculations/CDBG FY10.csv") %>%
  filter (TY %in% c("21", "51", "52", "66")) #keep only states, cities and counties

#convert ST/HPL to character and pad 0s to concatenate into UOGID
cdbg_allocation$ST <- str_pad (cdbg_allocation$ST, 2, pad = "0")
cdbg_allocation$HPL <- str_pad (cdbg_allocation$HPL, 4, pad = "0")
...
cdbg_total <- sum (cdbg_allocation$cdbg_amt) + 1183000000 
...
#Petersburg Borough added in FY12 and not in FY11 so will appear here
```
...
write.csv (esg_coc_crosswalk, "data/outputs/x12_simple_shares.csv")
```