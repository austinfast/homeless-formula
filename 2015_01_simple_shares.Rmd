...
anti_join (crosswalk, pprn_df, by = "UOGID") %>%
  filter(!(geo_name %in% c("Kusilvak Census Area", "Jurupa Valley", "Burien", "Oglala Lakota County", "019015")))
...
#   UOGID == "366468" ~ "369999", #Add Tonawanda Town to NY BoS (re-separated in 2015/2016)
    UOGID == "459063" ~ "459999", #Add Lexington County to SC BoS
#   UOGID == "261638" ~ "269999", #Add Dearborn to MI BoS (re-separated in 2015/2016)
...
#ESG additions for 2016
   ...
   # UOGID == "063348" ~ "063348", # Santa Barbara County goes back to CA BoS for 2015
...
#import ESG14 allocations from HUD to make list of eligible UOGIDs
hud_funding <- read_excel("data/inputs/hud_allocations/fy2014-formula-allocations-allgrantees.xlsx", skip = 1) %>%
  ...
  dplyr::select (1, 5, 8) %>%
  filter (esg14 != 0) %>% #finds only ESG eligible
...
#import ESG14 allocations from HUD to calculate total allocation
hud_funding <- read_excel("data/inputs/hud_allocations/fy2014-formula-allocations-allgrantees.xlsx", skip = 1) %>%
  clean_names() %>%
  rename(UOGID = key) %>%
  mutate (UOGID = if_else (name == "District Of Columbia", "119999", UOGID)) %>% 
...
esg_total <- sum (hud_funding$esg14) * .998
```
...
```{r}
cdbg_allocation <- read_excel ("data/inputs/hud_calculations/CDBG FY13.xlsx", sheet = "CALCS", 
                       n_max = 1234) %>% #extra row at end
  mutate (HPL = if_else (HPL == "0000", "9999", HPL),
          UOGID = paste0(ST, HPL)) %>%
  rename (cdbg_amt = "GA") %>% 
  dplyr::select (UOGID, NAME, cdbg_amt) 

#calculate total 2013 CDBG formula allocations
...
write.csv (esg_coc_crosswalk, "data/outputs/x15_simple_shares.csv")
```