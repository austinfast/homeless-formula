...
## Sources 
* FY 2013/14 Continuum of Care (CoC) Geographic Code Report
https://www.hudexchange.info/sites/onecpd/assets/File/FY2014-CoC-Program-GeoCodes-PPRN-Amounts.pdf
* FY2013 HUD funding allocation
https://www.hud.gov/program_offices/comm_planning/budget/fy14
* FY2012 CDBG calculation workbook (emailed from HUD)
...
```{r}
pprn_df <- read.csv("data/inputs/hud_allocations/FY-2014-Geographic-Codes-with-PPRN.csv") %>%
  clean_names() %>% 
...
anti_join (pprn_df, crosswalk, by = "UOGID") %>%
  ...
  filter(!(sta %in% c("AS","GU","MP","VI"))) #filter out insular areas 
```
...
#ESG removals for 2018
    ...
    UOGID == "194812" ~ "199999", #Sioux City goes back to IA BoS in 2014
    UOGID == "360744" ~ "369999", #Add Brookhaven town to NY BoS  
#ESG removals for 2017
    UOGID == "061750" ~ "069999", #Add Irvine to CA BoS
#   UOGID == "269081" ~ "269999", #Add Kent County to MI BoS (re-separated in 2015)
    UOGID == "366468" ~ "369999", #Add Tonawanda Town to NY BoS again
...
#ESG removals for 2014
    ...
    UOGID == "175154" ~ "179999", #Add Oak Park to IL BoS
...
#import ESG13 allocations from HUD to make list of eligible UOGIDs
hud_funding <- read_excel("data/inputs/hud_allocations/FY2013_ALLGRANTEES.XLS", skip = 1) %>% 
  ...
  dplyr::select (1, 5, 8) %>%
  filter (esg13 != 0) %>% #finds only ESG eligible
...
#import ESG13 allocations from HUD to calculate total allocation
hud_funding <- read_excel("data/inputs/hud_allocations/FY2013_ALLGRANTEES.XLS", skip = 1) %>% 
  clean_names() %>%
  rename(UOGID = key) %>%
  mutate (UOGID = if_else (name == "District Of Columbia", "119999", UOGID)) %>% 
  dplyr::select (UOGID, esg13)

#Calculate total ESG-entitled allocation
esg_total <- sum (hud_funding$esg13) * .998
```
...
```{r}
cdbg_allocation <- read_excel ("data/inputs/hud_calculations/CDBG FY12.xlsx", sheet = "CALCS") %>% 
  mutate (HPL = if_else (HPL == "0000", "9999", HPL),
          UOGID = paste0(ST, HPL)) %>%
  rename (cdbg_amt = "GA") %>% 
  dplyr::select (UOGID, NAME, cdbg_amt) 

#calculate total 2012 CDBG formula allocations
...
write.csv (esg_coc_crosswalk, "data/outputs/x14_simple_shares.csv")
```